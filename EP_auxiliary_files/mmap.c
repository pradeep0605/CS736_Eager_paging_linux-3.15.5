		...
		unsigned long, fd, unsigned long, pgoff)
{
	...
	|| ( mm &&  (mm->apriori_paging_en == 1)  &&  
		( ((flags & (MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED))  == ( MAP_PRIVATE
| MAP_ANONYMOUS )) 
		|| ( flags == MAP_PRIVATE )
                || ( flags == ( MAP_PRIVATE | MAP_POPULATE)) 
                ||  ( flags == ( MAP_PRIVATE | MAP_POPULATE | MAP_EXECUTABLE |
MAP_DENYWRITE)))))
		{
		int j;
...
//					printk("Found j:%d\n", j);
	...
	if(unlikely((mm->identity_mapping_en >= 1) && ((flags & MAP_FIXED) !=
MAP_FIXED) 
				&& (get_pa(retval) > 0))){ 
