...
SYSCALL_DEFINE2(apriori_paging_mfile_enable, pid_t, pid, int, condition)
{
    if ( condition > 0 ) {
        current->mm->apriori_paging_mfile_en = 1;
        printk(KERN_INFO "enable memory_mapped_files - pid: %d\n", pid);
    }
    else {
        current->mm->apriori_paging_mfile_en = 0;
        printk(KERN_INFO "disable memory_mapped_files - pid: %d\n", pid);
    }
