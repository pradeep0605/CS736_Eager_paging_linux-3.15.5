...
static int fill_page_table_manually_cow(struct mm_struct *mm , struct vm_area_struct *vma,
                                    unsigned long addr, unsigned long nr_pages, unsigned int flags)
{
    ...
    {
        if ( __pte_alloc(mm,vma,pmd,addr) )
    ...
    for ( i = 1 ; i < nr_pages ; i++ )
    {
        ...
        if (pmd_none(*pmd) || pmd_bad(*pmd)) {
            ...
            if ( __pte_alloc(mm,vma,pmd,new_addr) )
                printk(KERN_INFO "Pte couldn't be allocated");
        ...
        copy_user_highpage(new_page, fault_page, new_addr, vma);
        __SetPageUptodate(new_page);

        ptep = pte_offset_map_lock(mm, pmd, new_addr, &ptl);

        atomic_set(&new_page->_count,1);

        /* Make the page table entry as reserved for TLB miss tracking */
        if(mm && (mm->badger_trap_en==1) && (!(vma->vm_flags & FAULT_FLAG_INST))) {
...
//        }
        ...
        do_set_pte(vma, new_addr, new_page, ptep, true, true, flags);
        pte_unmap_unlock(ptep, ptl);
        unlock_page(fault_page);
        page_cache_release(fault_page);
...
static int fill_page_table_manually(struct mm_struct *mm , struct vm_area_struct *vma, unsigned long addr, unsigned long nr_pages)
{
    int i;
    pgd_t *pgd;
    pte_t *ptep ,pte;
    ...
    */
    ...
    for ( i = 1 ; i < nr_pages ; i++ )
    {
        ...
        */
        ...
        atomic_set(&temp->_count,1);

        mem_cgroup_charge_anon(temp, mm, GFP_KERNEL);
        ...
        page_add_new_anon_rmap(temp,vma,new_addr);
	...
	vma->apriori_en=1;
	if(mm && (mm->badger_trap_en==1) && (!(vma->vm_flags & FAULT_FLAG_INST)))
		add_page_to_rmt((new_addr & PAGE_MASK),(pte_val(pte) & PTE_PFN_MASK));
...
//                pzone = &node_0->node_zones[ZONE_DMA32];
		...
		if ((apriori_flag == 1) || (apriori_flag == 2)) {
                    ret = handle_mm_fault_apriori_paging(mm, vma, start, fault_flags, apriori_flag, apriori_order);
...
gotten:
	...
	if (likely(pte_same(*page_table, orig_pte))) {
		...
		entry = maybe_mkwrite(pte_mkdirty(entry), vma);
			 ...
			 add_page_to_rmt((address & PAGE_MASK),(pte_val(entry) & PTE_PFN_MASK));
                }
		...
		unsigned int flags, unsigned long apriori_flag, int apriori_order)
{
     ...
     */
    if (apriori_flag == 1)
        page = alloc_page_vma_apriori_paging(GFP_HIGHUSER | __GFP_ZERO | __GFP_MOVABLE, apriori_order, vma, address);
    else
...
void do_set_pte(struct vm_area_struct *vma, unsigned long address,
		struct page *page, pte_t *pte, bool write, bool anon, unsigned int flags)
{
	pte_t entry;
	...
	/* Make the page table entry as reserved for TLB miss tracking */
	if(vma->vm_mm && (vma->vm_mm->badger_trap_en==1) && (!(flags & FAULT_FLAG_INST)))
	{
        	entry = pte_mkreserve(entry);
		 add_page_to_rmt((address & PAGE_MASK),(pte_val(entry) & PTE_PFN_MASK));
        }
...
static int do_cow_fault(struct mm_struct *mm, struct vm_area_struct *vma,
	unsigned long address, pmd_t *pmd,
        pgoff_t pgoff, unsigned int flags, pte_t orig_pte,
        unsigned long apriori_flag, int apriori_order)
{
	...
	if (unlikely(anon_vma_prepare(vma)))
    	...
    	if (apriori_flag == 2) {
       		new_page = alloc_page_vma_apriori_paging(GFP_HIGHUSER_MOVABLE, apriori_order, vma, address);
    	}
    	else {
        	new_page = alloc_page_vma(GFP_HIGHUSER_MOVABLE, vma, address);
    	}
...
static int do_linear_fault(struct mm_struct *mm, struct vm_area_struct *vma,
	unsigned long address, pte_t *page_table, pmd_t *pmd,
        unsigned int flags, pte_t orig_pte, unsigned long apriori_flag, int apriori_order)
{
	...
	if (!(vma->vm_flags & VM_SHARED))
		return do_cow_fault(mm, vma, address, pmd, pgoff, flags,
				orig_pte, apriori_flag, apriori_order);
               ...
               unsigned int flags, spinlock_t *ptl)
{
	...
	if(rmc_access(address) == 1)
	{
		current->total_rmt_dtlb_misses++;
	}
	if(vtlb_access(address)==1)
	{
		current->total_vtlb_dtlb_misses++;
	}
	ctlb_access(address);
...
static int handle_pte_fault(struct mm_struct *mm,
		     ...
		     pte_t *pte, pmd_t *pmd, unsigned int flags, unsigned long apriori_flag, int apriori_order)
{
	pte_t entry;
	spinlock_t *ptl;
	...
	if (!pte_present(entry)) {
		if (pte_none(entry)) {
			if (vma->vm_ops) {
				...
				{
					if(mm->apriori_paging_en == 1)
					{
//					printk("Linear Fault Address: %lx\n",address);
					vma->apriori_en = 1;
					}
					return do_linear_fault(mm, vma, address,
						pte, pmd, flags, entry, apriori_flag, apriori_order);
				}
			}
			return do_anonymous_page(mm, vma, address,
						 pte, pmd, flags, apriori_flag, apriori_order);
		}
		if (pte_file(entry))
		{
			if(mm->apriori_paging_en == 1)
//			printk("Nonlinear Fault Address: %lx\n",address);
			return do_nonlinear_fault(mm, vma, address,
					pte, pmd, flags, entry);
		}
		if(mm->apriori_paging_en == 1)
//		printk("Swap Page Address: %lx\n",address);
...
escape:
	...
	return handle_pte_fault(mm, vma, address, pte, pmd, flags, apriori_flag, apriori_order);
}
		    ...
		    unsigned long address, unsigned int flags)
{
	...
	ret = __handle_mm_fault(mm, vma, address, flags, apriori_flag, apriori_order);
...
int handle_mm_fault_apriori_paging(struct mm_struct *mm, struct vm_area_struct *vma,
	    unsigned long address, unsigned int flags, unsigned long apriori_flag, int apriori_order)
{
       int ret;
