...
unsigned long do_mmap_pgoff(struct file *file, unsigned long addr,
			...
			unsigned long *populate, unsigned long *apriori_flag)
{
	struct mm_struct * mm = current->mm;
	vm_flags_t vm_flags;
	...
	*apriori_flag = 0;
    ...
    if (!IS_ERR_VALUE(addr)) {
        if ((( flags &  MAP_APRIORI_PAGING ) ==  MAP_APRIORI_PAGING ) || 
			...
			MAP_ANONYMOUS | MAP_FIXED)) == (MAP_PRIVATE | MAP_ANONYMOUS))))
//        if ( (( flags &  MAP_APRIORI_PAGING ) ==  MAP_APRIORI_PAGING ) || ( mm &&  mm->apriori_paging_en == 1 && (!((flags & (MAP_FIXED | MAP_PRIVATE | MAP_DENYWRITE)) == (MAP_FIXED | MAP_PRIVATE | MAP_DENYWRITE)))))
		...
		else if( mm &&  mm->apriori_paging_en == 1  && ((flags & (MAP_EXECUTABLE |
			MAP_DENYWRITE | MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED) ) ==
...
find_extend_vma(struct mm_struct *mm, unsigned long addr)
{
	...
	if (prev->vm_flags & VM_LOCKED)
		__mlock_vma_pages_range(prev, addr, prev->vm_end, NULL, apriori_flag);
	return prev;
}
#else
int expand_stack(struct vm_area_struct *vma, unsigned long address)
...
find_extend_vma(struct mm_struct * mm, unsigned long addr)
{
	...
	if (expand_stack(vma, addr))
		return NULL;
	if (vma->vm_flags & VM_LOCKED)
		__mlock_vma_pages_range(vma, addr, start, NULL, apriori_flag);
	return vma;
}
#endif
